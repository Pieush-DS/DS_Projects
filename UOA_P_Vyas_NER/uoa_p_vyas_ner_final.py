# -*- coding: utf-8 -*-
"""UOA_P_Vyas_NER_Assignment.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1lcSDRLhWUeX79zVPu109GrMsrAwjFInZ

# Identifying Entities in Healthcare Data

# <font color = blue> NER </font>
### **Problem Statement:**
...
- Therefore, to find out the disease and its probable treatment from the given dataset and store it in the form of a table or a dictionary is necessary to make it easier for the non-medical bacground people.
- After analyzing the problem need to build a custom NER to get the list of diseases and their treatment from the data set.

**Steps to follow:**

- To process and modify the data into sentence format. This has to be done for the 'train_sent' and ‘train_label’ datasets and for the test datasets too.
- Then, need to define the features to build the CRF model.
- After that, need to apply these features in each sentence of the train and the test dataset to get the feature values.
- Once the features are computed, it is required to define the target variable and then build the CRF model.
- Further, need to perform the evaluation using a test data set.
- At the end, it is required to create a dictionary in which diseases are keys and treatments are values.
"""
...
# Import all the datasets and read the lines
dir = "/content/drive/MyDrive/UOA/NER"

with open(dir + "/train_sent", 'r') as train_sent_lines:
 train_lines = train_sent_lines.readlines()

with open(dir + "/test_sent", 'r') as test_sent_lines:
  test_lines = test_sent_lines.readlines()

with open(dir + "/train_label", 'r') as train_labels_lines:
  train_labels_lines = train_labels_lines.readlines()

with open(dir + "/test_label", 'r') as test_labels_lines:
  test_labels_lines = test_labels_lines.readlines()

# To get 'train_lines' words
train_lines

# To get 'test_lines' words
test_lines

# To get 'train_labels' words
train_labels_lines

# To get 'test_labels' words
test_labels_lines

# To define a function to construct sentences from individual words

# Function to process raw sentence lines
def process_sentence(sentence_lines):
    sentences_list = []
    sentence = []
    for word in sentence_lines:
        if "\n" == word:
...
# Function to process raw label lines
def process_labels(label_lines):
    sentences_list = []
    sentence = ""

    for ch in label_lines:
        ch = ch.strip()
        if ch == "":
...
# To apply above function to convert lines to sentences
train_sentences = process_sentence(train_lines)
test_sentences = process_sentence(test_lines)

train_labels = process_sentence(train_labels_lines)
test_labels = process_sentence(test_labels_lines)

# To print 1st 5 Train Sentences with their labels.
print("First 5 Train Sentences with their labels:\n")
train_sentences[:5]

# To print 1st 5 Test sentences with their labels.
print("First 5 Test Sentences with their labels:\n")
test_sentences[:5]

"""### Count the number of sentences in the processed train and test dataset"""

# To count number of sentences in Train dataset
print(f"There are total {len(train_sentences)} sentences in train dataset")

# To count number of sentences in Test dataset
print(f"There are total {len(test_sentences)} sentences in test dataset")

"""### Count the number of lines of labels in the processed train and test dataset."""

# To count number of lines of labels in Train dataset
print(f"There are total {len(train_labels)} labels' lines in train dataset")

# To count number of lines of labels in Test dataset
print(f"There are total {len(test_labels)} labels' lines in test labels")
...
# To get Noun and Propn
train_test_sentences = train_sentences + test_sentences
tag_nouns = []

for review in tqdm((" ".join(train_test_sentences)).split(" ")):
    nlp = model(review)
    for token in nlp:
        if token.pos_=="NOUN" or token.pos_=="PROPN":
            tag_nouns.append(token.text)

"""### Print the top 25 most common tokens with NOUN or PROPN PoS tags"""

# To get top 25 most common tokens
pd.Series(tag_nouns).value_counts().head(25)
...
# Apply function 'getFeaturesForOneSentence' to get features just on a single sentence which is at index value 15 in 'all_train_sentences'
ex_sent = train_sentences[15]
print(ex_sent)

features = getFeaturesForOneSentence(ex_sent)
features[2]
...
# To get the train and test sentences datsaet
X_train = [getFeaturesForOneSentence(sentence) for sentence in train_sentences]
X_test = [getFeaturesForOneSentence(sentence) for sentence in test_sentences]

"""### Define the labels as the target variable for test and the train dataset"""

# To get the train and test labels datsaet
...
# Predict the labels of each tokens for the test data
Y_pred = crf.predict(X_test)

"""### Calculate the f1 score using the actual labels and the predicted labels of the test dataset."""

# To get f1-Score value
f1_score = metrics.flat_f1_score(Y_test, Y_pred, average='weighted')
print("F-1 Score: ", round((f1_score*100),2))

"""##### F-1 score fro this model is **90.44%**
...
# To define function to create dictionary
def disease_treat(X_dict, Y_dict):
    diseases_all = []
    treatments_all = []
    m = 0
    for titels in Y_pred:
        n = 0
        diseases_titles = []
        for titel in titels:
            if titel == "D":
               diseases_titles.append(X_test[m][n][0].split("=")[-1])
            elif titel == "T":
               treatments_all.append([X_test[m][n][0].split("=")[-1], m])
            else:
                if len(diseases_titles)!=0:
                   diseases_all.append([diseases_titles, m])
                   diseases_titles = []
            n=n+1
        if len(diseases_titles)!= 0:
           diseases_all.append([diseases_titles, m])
        m=m+1
    return diseases_all, treatments_all

X_dict = X_test
Y_dict = Y_pred

diseases_all, treatments_all = disease_treat(X_dict, Y_dict)

# To get complete dictionary output
diseases_treatments_dict = {}
for diseases in diseases_all:
  if " ".join(diseases[0]) not in diseases_treatments_dict.keys():
     diseases_treatments_dict[" ".join(diseases[0])] = []
  for item in treatments_all:
      treatments = []
      if item[1] == diseases[1]:
         diseases_treatments_dict[" ".join(diseases[0])].append(item[0])
diseases_treatments_dict

"""### Predict the treatment for the disease name: 'hereditary retinoblastoma'"""

# To get treatment for the disease "hereditary retinoblastoma"
diseases_treatments_dict["hereditary retinoblastoma"]

"""#### Inferences:
- After analysis with the availabble data set the treatment for the disease **"hereditary retinoblastoma"** founded is **"radiotherapy"**.

#### <font color = black> End of NER Assignment related to "Identifying Entities in Healthcare Data"</font>
"""